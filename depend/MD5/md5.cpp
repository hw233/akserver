...
namespace ak
{
    ...
    void GenerateFileMD5(char *pFilePath,char *pCode)
    {
        memset(pCode,0,16);
...
#define GEN_FILE_MD5_BUF_SIZE 1024*1024
            char *pBuf=new char [GEN_FILE_MD5_BUF_SIZE];

            fseek(fp,0,2);
            unsigned int uFileSize=ftell(fp);

            unsigned int uReadSize=0;
            fseek(fp,0,0);
            for(unsigned int i=0;i<uFileSize;i+=uReadSize)
            {
                uReadSize=uFileSize-i;
                if(uReadSize>GEN_FILE_MD5_BUF_SIZE)uReadSize=GEN_FILE_MD5_BUF_SIZE;

                fread(pBuf,uReadSize,1,fp);

                md5.addData(pBuf,uReadSize);
            }
